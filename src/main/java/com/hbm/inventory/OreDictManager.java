...
public class OreDictManager {
	...
	public static void registerOres() {
		 ...
		 * RARE EARTHS
		 */
		RAREEARTH.ingot(DictFrame.fromOne(chunk_ore, EnumChunkType.RARE)).ore(ore_rare, ore_gneiss_rare);
		...
		for(int i = 0; i < 16; i++) { ANY_CONCRETE.any(new ItemStack(concrete_colored, 1, i)); }
		for(int i = 0; i < 16; i++) { ANY_CONCRETE.any(new ItemStack(concrete_colored_ext, 1, i)); }
		...
		ANY_ASH					.any(fromOne(powder_ash, EnumAshType.WOOD), fromOne(powder_ash, EnumAshType.COAL), fromOne(powder_ash, EnumAshType.MISC), fromOne(powder_ash, EnumAshType.FLY), fromOne(powder_ash, EnumAshType.SOOT));
		...
		for(NTMMaterial mat : Mats.orderedList) {
			if(mat.smeltable == SmeltingBehavior.SMELTABLE) {
				if(mat.autogen.contains(CASTPLATE)) for(String name : mat.names) OreDictionary.registerOre(CASTPLATE.name() + name, new ItemStack(plate_cast, 1, mat.id));
				if(mat.autogen.contains(WELDEDPLATE)) for(String name : mat.names) OreDictionary.registerOre(WELDEDPLATE.name() + name, new ItemStack(plate_welded, 1, mat.id));
				if(mat.autogen.contains(HEAVY_COMPONENT)) for(String name : mat.names) OreDictionary.registerOre(HEAVY_COMPONENT.name() + name, new ItemStack(heavy_component, 1, mat.id));
				if(mat.autogen.contains(DENSEWIRE)) for(String name : mat.names) OreDictionary.registerOre(DENSEWIRE.name() + name, new ItemStack(wire_dense, 1, mat.id));
			}
			if(mat.autogen.contains(BOLT)) for(String name : mat.names) OreDictionary.registerOre(BOLT.name() + name, new ItemStack(bolt, 1, mat.id));
			if(mat.autogen.contains(INGOT)) for(String name : mat.names) OreDictionary.registerOre(INGOT.name() + name, new ItemStack(ingot_raw, 1, mat.id));
			if(mat.autogen.contains(SHELL)) for(String name : mat.names) OreDictionary.registerOre(SHELL.name() + name, new ItemStack(shell, 1, mat.id));
			if(mat.autogen.contains(PIPE)) for(String name : mat.names) OreDictionary.registerOre(PIPE.name() + name, new ItemStack(pipe, 1, mat.id));
			if(mat.autogen.contains(FRAGMENT)) for(String name : mat.names) OreDictionary.registerOre(FRAGMENT.name() + name, new ItemStack(bedrock_ore_fragment, 1, mat.id));
			if(mat.autogen.contains(WIRE)) for(String name : mat.names) OreDictionary.registerOre(WIRE.name() + name, new ItemStack(wire_fine, 1, mat.id));

			if(mat.autogen.contains(LIGHTBARREL)) for(String name : mat.names) OreDictionary.registerOre(LIGHTBARREL.name() + name, new ItemStack(part_barrel_light, 1, mat.id));
			if(mat.autogen.contains(HEAVYBARREL)) for(String name : mat.names) OreDictionary.registerOre(HEAVYBARREL.name() + name, new ItemStack(part_barrel_heavy, 1, mat.id));
			if(mat.autogen.contains(LIGHTRECEIVER)) for(String name : mat.names) OreDictionary.registerOre(LIGHTRECEIVER.name() + name, new ItemStack(part_receiver_light, 1, mat.id));
			if(mat.autogen.contains(HEAVYRECEIVER)) for(String name : mat.names) OreDictionary.registerOre(HEAVYRECEIVER.name() + name, new ItemStack(part_receiver_heavy, 1, mat.id));
			if(mat.autogen.contains(MECHANISM)) for(String name : mat.names) OreDictionary.registerOre(MECHANISM.name() + name, new ItemStack(part_mechanism, 1, mat.id));
			if(mat.autogen.contains(STOCK)) for(String name : mat.names) OreDictionary.registerOre(STOCK.name() + name, new ItemStack(part_stock, 1, mat.id));
			if(mat.autogen.contains(GRIP)) for(String name : mat.names) OreDictionary.registerOre(GRIP.name() + name, new ItemStack(part_grip, 1, mat.id));
		}
		
		for(EnumBedrockOre ore : EnumBedrockOre.values()) {
			BlockTags.registerOre("ore" + ore.oreName, new ItemStack(ore_enriched, 1, ore.ordinal()));
		}
		...
		for(int i = 0; i < 16; i++) {
			...
			OreDictionary.registerOre(dyeName, new ItemStack(chemical_dye, 1, i));
			
			OreDictionary.registerOre(dyeName, new ItemStack(crayon, 1, i));
		}
		...
		for(NTMMaterial mat : Mats.orderedList) {
			if(mat.autogen.contains(FRAGMENT)) {
				String name = mat.names[0];
				if(!OreDictionary.getOres(DUST.name() + name).isEmpty()) MineralRecipes.add9To1(mat.make(bedrock_ore_fragment), OreDictionary.getOres(DUST.name() + name).get(0));
				else if(!OreDictionary.getOres(GEM.name() + name).isEmpty()) MineralRecipes.add9To1(mat.make(bedrock_ore_fragment), OreDictionary.getOres(GEM.name() + name).get(0));
				else if(!OreDictionary.getOres(CRYSTAL.name() + name).isEmpty()) MineralRecipes.add9To1(mat.make(bedrock_ore_fragment), OreDictionary.getOres(CRYSTAL.name() + name).get(0));
				else if(!OreDictionary.getOres(INGOT.name() + name).isEmpty()) MineralRecipes.add9To1(mat.make(bedrock_ore_fragment), OreDictionary.getOres(INGOT.name() + name).get(0));
				else if(!OreDictionary.getOres(BILLET.name() + name).isEmpty()) MineralRecipes.addBilletFragment(OreDictionary.getOres(BILLET.name() + name).get(0), mat.make(bedrock_ore_fragment));
				else MineralRecipes.add9To1(mat.make(bedrock_ore_fragment), new ItemStack(nothing));
			}
		}
