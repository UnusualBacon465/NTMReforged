...
public class PipeNet implements IPipeNet {
	...
	private final List<IFluidConductor> links = new ArrayList<>();
	private final Set<IFluidConnector> subscribers = new HashSet<>();
	...
	public long transferFluid(long fill, int pressure) {
		subscribers.removeIf(connector ->
				connector == null || !(connector instanceof BlockEntity blockEntity) || blockEntity.isRemoved() || !connector.isLoaded()
		);

		if (this.subscribers.isEmpty()) return fill;
	...
	public static long fairTransfer(List<IFluidConnector> subList, FluidType type, int pressure, long fill) {
		...
		for (int i = 0; i < subList.size(); i++) {
			...
			double fraction = (double) req / totalReq;
			...
			if (given > 0) {
				...
				if (con instanceof BlockEntity blockEntity) {
					blockEntity.setChanged();
				}

				if (trackingInstances != null) {
					for (PipeNet net : trackingInstances) {
						log(net, currentTime() + " Sending " + given + "mB to " + conToString(con));
					}
				}
			}
		}
	...
	public static String conToString(IFluidConnector con) {
		if (con instanceof BlockEntity blockEntity) {
			...
			return blockEntity.getClass().getSimpleName() + " @ " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ();
		}
